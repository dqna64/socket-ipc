...
class Publisher:
    ...
    def __accept_subscriptions(self):
        ...
        while not self.stop_accepting_subscriptions.is_set():
            try:
                ...
                DEBUG and print(f"New subscriber connection to publisher accepted: {addr}")
    ...
    def __add_subcription(self, addr, subscription: Subscription):
        with self.subscriptions_lock:
            ...
            DEBUG and print(f"Added subscription to socket {addr}")

    def __remove_subscription(self, addr):
        with self.subscriptions_lock:
            del self.subscriptions[addr]
            DEBUG and print(f"Removed subscription to socket {addr}")
    ...
    def close(self):
        ...
        DEBUG and print(f"Waiting for thread {self.accept_subscriptions_thread.name} handling accepting new subscribers to finish...")
        self.accept_subscriptions_thread.join()
        DEBUG and print(f"Thread {self.accept_subscriptions_thread.name} handling accepting new subscribers has finished.")
